...
try {
    ...
    if ($codMarca) {
        ...
        if ($codModelo) {
            $anos = FipeCaminhoes::getAnos($codMarca, $codModelo);
            if (!$anos) {
        ...
        if ($codAno) {
            $veiculo = FipeCaminhoes::getVeiculo($codMarca, $codModelo, $codAno);
            if (!$veiculo) {
...
<body>
<p>Exemplo de consulta de Caminhões na Fipe<?php if (!$codMarca) {
